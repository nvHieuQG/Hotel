    ...
    <div class="container">
        <div class="row no-gutters slider-text d-flex align-itemd-end justify-content-center">
            <div class="col-md-9 ftco-animate text-center d-flex align-items-end justify-content-center">
                <div class="text">
                    <p class="breadcrumbs mb-2"><span class="mr-2"><a href="{{ route('index') }}">Trang chủ</a></span> <span>Đặt Phòng</span></p>
                ...
                <div class="bg-white p-5 contact-form w-100">
                    ...
                    <form action="{{ route('booking') }}" method="post">
                            ...
                            <select name="room_id" id="room_id" class="form-control" required>
                                ...
                                @foreach ($rooms as $room)
                                    <option value="{{ $room->id }}" data-price="{{ $room->roomType->price }}">
                                        {{ $room->name }} - {{ number_format($room->roomType->price) }}đ/đêm
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="date" name="check_in" id="check_in" class="form-control" required min="{{ date('Y-m-d') }}" value="{{ old('check_in') }}">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="check_out">Ngày Trả Phòng</label>
                                    <input type="date" name="check_out" id="check_out" class="form-control" required min="{{ date('Y-m-d', strtotime('+1 day')) }}" value="{{ old('check_out') }}">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" disabled value="{{ $user->name }} ({{ $user->email }})">
                        </div>
